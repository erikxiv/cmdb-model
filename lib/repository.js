...
function persistRepository(repo, change) {
  if (repo.options.persist) {
    fs.writeFile('cmdb-'+repo.name+'-c'+change.revision+'.json', JSON.stringify(change));
    repo.store.graph(repo.ns, function(status, graph){
      // console.log(graph.toNT().split('\n').sort().join('\n'));
      fs.writeFile('cmdb-'+repo.name+'.json',graph.toNT().split('\n').sort().join('\n'));
    });
  }
}
